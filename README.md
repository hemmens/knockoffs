# knockoffs
The code in this repository accompanies the Robert and Hemmens paper about using linear algebra to derive Model-X Knockoffs, as originally described in Candès (2018).

## knockoff_lib.py
This is a library we use to perform a number of actions:

- Calculate the coskewness and cokurtosis matrices for a collection of random variables. These actions are not available in the scipy package, in which only the skewness and kurtosis for a single variable are available.
